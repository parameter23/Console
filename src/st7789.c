...
{
    /* Originally DIV_2 (50 MHz on this project's 100 MHz APB2) before
     * this driver was retired in favour of ILI9488. That was never
     * hardened against the signal-integrity/power-supply issues found
     * later while bringing up the ILI9488 panel (see ili9488.c history:
     * corruption there needed DIV_8, 12.5 MHz, to go away even on
     * shorter wiring than this ST7789 module uses). DIV_2 is restored
     * here as-is since this is a straight revert, but watch for the
     * same stray-pixel/color-shift corruption on real hardware - if it
     * shows up, drop this divider the same way (DIV_4, then DIV_8). */
