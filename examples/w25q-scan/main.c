/**
 * @file main.c
 * @brief Auto-detect which SPI bus/CS pin an onboard W25Qxx flash is
 ...
 *        reports which one (if any) sees a genuine Winbond part
 *        (JEDEC manufacturer ID 0xEF, memory type 0x40 - the capacity
 *        byte is deliberately not checked here, so this works for a
 *        W25Q80, W25Q128 or any other family member).
 *
...
/** @brief Configures one candidate bus/CS combo and checks for a
 *         Winbond JEDEC ID (manufacturer 0xEF, memory type 0x40). */
