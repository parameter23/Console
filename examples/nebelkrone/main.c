/**
 * @file main.c
 * @brief "Die Nebelkrone" - an illustrated interactive-fiction gamebook
 *        built on the generic GameAPI, in the spirit of the solo
 *        gamebooks Das Schwarze Auge itself published: numbered choices
 *        instead of a text parser (this hardware has a joystick, not a
 *        keyboard), and single-die attribute checks - roll a d20 against
 *        one of three hero attributes (MUT/KLUGHEIT/GEWANDTHEIT) - as a
 *        compact stand-in for the full pen-and-paper ruleset. An original
 *        story (see story.c), not a reproduction of any published DSA
 *        adventure.
 *
...
static void do_combat_action(int action)
{
    ...
    if (action == 2) { /* TRANK */
        ...
        hero.flags &= ~(1u << FLAG_POTION);
        sfx_play(SFX_PICKUP);
        append(display_text, &p, "Du trinkst den Heiltrank. (+6 LP)");
    } else { /* ATTACKE */
        int roll = d20();
    ...
    if (eroll <= e->hit_chance) {
        ...
        append(display_text, &p, " LP).");
        display_text[p] = 0;
        if (apply_damage(edmg, "Der Moorschrat ueberwaeltigt dich im Kampf."))
            return;
    } else {
        append(display_text, &p, " Der ");
        append(display_text, &p, e->name);
...
static void nebel_update(const JoystickState *js, uint32_t tick_ms)
{
    (void)tick_ms;
    ...
    case STATE_SPIEL: {
        ...
        case NODE_COMBAT: {
            int has_potion = (hero.flags & (1u << FLAG_POTION)) != 0;
            int count = has_potion ? 3 : 2;
...
{
    draw_center(30,  "DIE NEBELKRONE", 5);
    draw_center(56,  "EIN ILLUSTRIERTES HELDENABENTEUER", 15);
    draw_center(88,  "IM STIL VON PEN-AND-PAPER-ROLLENSPIELEN:", 1);
    draw_center(102, "PROBEN AUF MUT, KLUGHEIT UND GEWANDTHEIT", 1);
    draw_center(116, "ENTSCHEIDEN UEBER DEIN SCHICKSAL.", 1);
...
static void draw_ende(void)
{
    ...
    if (!ende_is_win && (hero.flags & (1u << FLAG_CROWN)))
        draw_center(HUD_Y - 12, "SO NAH - DIE KRONE WAR SCHON DEIN.", 7);

    draw_center(HUD_Y, "DRUECKE DEN KNOPF FUER NEUSTART", 15);
}

/** @brief GameAPI draw callback. */
static void nebel_draw(void)
{
    ...
    case NODE_COMBAT: {
        draw_combat_status();
        int has_potion = (hero.flags & (1u << FLAG_POTION)) != 0;
        static const char *const labels[3] = { "ATTACKE", "FLIEHEN", "TRANK (HEILT 6 LP)" };
        int count = has_potion ? 3 : 2;
...
static const GameAPI nebelkrone_game = {
    .name   = "nebelkrone",
    .init   = nebel_init,
    .update = nebel_update,
    .draw   = nebel_draw,
...
/** @brief Entry point: runs Die Nebelkrone via the generic GameAPI loop. */
int main(void)
{
    game_run(&nebelkrone_game, TICK_MS);
    return 0;
}
